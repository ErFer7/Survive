 ...
 */
void FreeGameplayContext(GameplayContext *gameplayCtxPtr) { FreeEntityMatrix(&gameplayCtxPtr->entityMatrix); }
...
void InsertEntityOnMatrix(EntityMatrix *entityMatrixPtr, Entity entity, Vector2D position) {
    ...
    if (entity.type == PLAYER) {
        ...
        if (entityMatrixPtr->coinPtrsSize == 0) {
            ...
            entityMatrixPtr->coinPtrs =
                (Entity **)realloc(entityMatrixPtr->coinPtrs, sizeof(Entity *) * (entityMatrixPtr->coinPtrsSize + 1));
        }

        entityMatrixPtr->coinPtrsSize++;
        entityMatrixPtr->coinPtrs[entityMatrixPtr->coinPtrsSize - 1] =
            GetEntityPtrFromMatrix(entityMatrixPtr, position);
        ...
        if (entityMatrixPtr->enemyPtrsSize == 0) {
            ...
            entityMatrixPtr->enemyPtrs =
                (Entity **)realloc(entityMatrixPtr->enemyPtrs, sizeof(Entity *) * (entityMatrixPtr->enemyPtrsSize + 1));
        }

        entityMatrixPtr->enemyPtrsSize++;
        entityMatrixPtr->enemyPtrs[entityMatrixPtr->enemyPtrsSize - 1] =
            GetEntityPtrFromMatrix(entityMatrixPtr, position);
    }
}
...
Entity CreateEnemy(Vector2D position) {
    Entity enemy = {.c = "\xB2\xB1\xB0\xB2",
                    .animationFrame = 0.0f,
                    .animationSpeed = ANIMATION_SPEED,
                    .isAnimated = 1,
                    .color = 0x0C,
...
// TODO: The Chronometer here can be local to this thread
void *Update(void *updateThreadArgPtr) {
    ...
    Chronometer *timeCtxPtr = ((UpdateThreadArg *)updateThreadArgPtr)->timeCtxPtr;

    double update_elapsed_time = 0.0;

    while (eventStateCtxPtr->state == GAMEPLAY) {
        start_chronometer(timeCtxPtr);

        UpdateEntityBehaviour(&gameplayCtxPtr->entityMatrix, &interfaceCtxPtr->gameplay);
        UpdateEntityPhysics(eventStateCtxPtr, gameplayCtxPtr, interfaceCtxPtr, update_elapsed_time);

        update_elapsed_time = stop_chronometer(timeCtxPtr);
        SetGameplayTextd(&interfaceCtxPtr->gameplay.texts[3], 1000000.0 / update_elapsed_time);
    }
...
// TODO: The Chronometer here isn't even used!
...
void UpdateEntityPhysics(EventStateContext *eventStateCtxPtr,
    ...
    incrementScore = UpdatePlayerPhysics(gameplayCtxPtr, update_elapsed_time);
    ...
    for (int i = 0; i < gameplayCtxPtr->entityMatrix.enemyPtrsSize; i++) {
        gameover = UpdateEnemyPhysics(gameplayCtxPtr, gameplayCtxPtr->entityMatrix.enemyPtrs[i], update_elapsed_time);
    ...
    if ((float)entityMatrixPtr->playerPtr->movementAccumulator >
        ...
        for (int j = (int)target.x - 1; j < (int)target.x + 2; j++) {
            for (int k = (int)target.y - 1; k < (int)target.y + 2; k++) {
                if ((j != (int)target.x || k != (int)target.y) && (j >= 0 && j < entityMatrixPtr->width) &&
...
void *RenderEntities(void *renderThreadArgPtr) {
    ...
    TerminalContext *consoleCtxPtr = ((RenderThreadArg *)renderThreadArgPtr)->consoleCtxPtr;
    ...
    while (*statePtr == GAMEPLAY) {
        ...
        for (int i = 0; i < consoleCtxPtr->size.y - 1; i++) {
            for (int j = 0; j < consoleCtxPtr->size.x; j++) {
                ...
                } else  // Coloca o caractere
                {
                    ...
                    if (isAnimated) {
                        entityPtr->animationFrame +=
                            ANIMATION_SPEED * ((float)(timeCtxPtr->renderingElapsedTime / 1000000.0));

                        if ((int)entityPtr->animationFrame > MAX_ANIM_FRAMES - 1) entityPtr->animationFrame = 0.0f;
        ...
        timeCtxPtr->renderingElapsedTime =
            StopStopWatch(timeCtxPtr->renderingFrequency, timeCtxPtr->renderingInitialTime);
